        ...
        if (curAppBrowser) {
            ...
            browser = await Promise.race([
                ...
                new Promise<never>((_resolve, reject) =>
                    setTimeout(() => reject(new Error(`连接之前创建的浏览器失败：${curAppBrowser.wsUrl}，超时：30秒`)), 30000)
                )
            ]);
            ...
            browser = await Promise.race([
                ...
                new Promise<never>((_resolve, reject) =>
                    setTimeout(() => reject(new Error(`连接之前创建的浏览器失败：${browserJson.wsUrl}，超时：30秒`)), 30000)
                )
            ]);
        }
    } catch (error) {
        console.error('复用浏览器失败:', error);
        ...
        const childCode = `
            ...
            (async ()=>{
                try {
                    ...
                    try { fs.unlinkSync('${tempFilePath.replace(/\\/g, '/')}'); } catch(e2) {}
                    process.exit();
                }
            })()
        `;
