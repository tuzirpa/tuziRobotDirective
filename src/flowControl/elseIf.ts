...
export const config: DirectiveTree = {
    name: 'flowControl.elseIf',
    key: 'flowControl.testElseIf',
    displayName: 'Else If 条件',
    icon: 'icon-web-create',
    sort: 10,
    isControl: true,
    isControlEnd: true,
    comment: '否则判断${operand1} ${operator} ${operand2} 是否成立，如果成立，则执行以下操作',
    inputs: {
        operand1: {
            name: '条件操作数1',
            value: '',
    ...
    async toCode(directive, block) {
        ...
        return `} else if (await robotUtil.system.flowControl.testElseIf( ${typeToCode(operand1)}, '${
            operator.value
