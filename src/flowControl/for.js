...
exports.config = {
	...
	isControlEnd: false,
	comment:
		"从${startIndex}开始循环到${endIndex}结束，步长为${step}，循环位置保存至${index}",
	inputs: {
		startIndex: {
			name: "开始数值",
			value: "1",
	...
	async toCode(directive, block) {
		...
		return `for (let ${
			index.name
		} of await robotUtil.system.flowControl.rangeIterator(Number(${typeToCode(
			startIndex
		)}), Number(${typeToCode(endIndex)}), Number(${typeToCode(
			step
		)}),${block})) {`;
	},
};
