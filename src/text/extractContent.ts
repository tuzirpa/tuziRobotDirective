...
export const config: DirectiveTree = {
    ...
    async toCode(directive, _block) {
        ...
        const { text, extractionWay, isMatchFirst, isCaseSensitive, regexpValue } =
            directive.inputs;
        const code = `var ${
            matchResult.name
            ...
            /\\/g,
            '\\\\'
        )}",regexpValue: "${regexpValue.value.replace(/\\/g, '\\\\')}",isMatchFirst: ${typeToCode(
            isMatchFirst
...
export const impl = async function ({
    ...
    if (text) {
        ...
        return isMatchFirst && match ? match[0] : '';
    }
};
